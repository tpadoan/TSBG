...
def getMoves(police, det_id):
    return (
        [(c, "cart") for c in cart[police[det_id]] if c not in police]
        + [(t, "tram") for t in tram[police[det_id]] if t not in police]
        + [(b, "boat") for b in boat[police[det_id]] if b not in police]
    )
...
detectives_model = MaskablePPO.load(
    "/home/anagen/students/nodm/main_spt9u/units/main/anagen/bassoda/TSBG/models/SB3_detectives/Masked_PPO_SY_OBS_EVERY_3_5.243M_10turns_3detectives_smartMRX_randomStartEachEpisode/final_model.zip",
    env=vec_env,
)
